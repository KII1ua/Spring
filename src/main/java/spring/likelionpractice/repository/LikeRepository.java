...
public interface LikeRepository {
    ...
    public Long findByArticleIdLikeCount(Long articleId);         // 게시글 하나 불러올 때 해다 게시글의 좋아요 개수 불러오기, 만약 로그인한 상태라면 해당 유저가 해당 게시글에 좋아요를 누른 상태인지도 조회

    public List<Article> findByArticleLikeMe(Member member);  // 내가 좋아요한 게시물 목록 조회

    public List<Article> findByArticleLikeCount(Article article);        // 게시글 리스트 불러올 때 각 게시글별 좋아요 개수 불러오기
